...
import { updateGas ,DeleteGas} from '../../../services/gasService';

export default function UpdateGasDetails(props) {
    ...
    const sendData = async() =>{
        ...
        let response=await updateGas (props.data._id,newupdateGas );
        ...
        if(response.ok){
            ToastAndroid.show(
                ...
                'Gas Station Successfully Updated..', ToastAndroid.SHORT
              );
...
const deleteGasStation=async()=>{
    let response = await DeleteGas(props.data._id);


    if(response.ok){
        ToastAndroid.show(

            'Gas Station Successfully Deleted..', ToastAndroid.SHORT
          );
  ...
  return (
    <View style={styles.centeredView}>
        ...
        <View style={{ marginTop: 10 }} >
                    ...
                    data={avdata}
                    setSelected={setAvaialability} 
                    defaultOption={{ key:availability, value:availability }}
                    />
